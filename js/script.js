...
function initDealers(filter) {

  var dealersRequest = new XMLHttpRequest();
  dealersRequest.open('GET', 'https://maps.gb-group.co/data/dealers.geojson');
  dealersRequest.onreadystatechange = function (dealers) {
      if (dealersRequest.readyState === 4) {
        dealers = JSON.parse(dealersRequest.response)
        initMap(dealers, filter)
      }
  };
  dealersRequest.send();
}
...
function initMap(dealers, filter) {
  console.log(filter)
...
const setMarkers = (dealers, map, filter) => {
